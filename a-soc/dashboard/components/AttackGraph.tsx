...
import { Globe, User, Database, Server } from "lucide-react";
...
export function AttackGraph({ data }: { data: GraphData }) {
  ...
  data.nodes.forEach((node, i) => {
    if (!positions[node.id]) {
      positions[node.id] = { x: 500, y: 80 + i * 70 };
    }
  });
    ...
    <div className="w-full h-full relative bg-slate-950/50 rounded-xl border border-slate-800 overflow-hidden">
      ...
      <svg className="w-full h-full min-h-[300px]" viewBox="0 0 600 300">
        ...
        {data.edges.map((edge, i) => {
          ...
          return (
            <g key={i}>
              <line x1={start.x} y1={start.y} x2={end.x} y2={end.y} stroke="#475569" strokeWidth="2" markerEnd="url(#arrowhead)" />
              <text x={(start.x + end.x) / 2} y={(start.y + end.y) / 2 - 10} textAnchor="middle" fill="#94a3b8" fontSize="10" className="font-mono uppercase">
                {edge.label}
              </text>
            </g>
          );
        ...
        {data.nodes.map((node, i) => {
          ...
          return (
            <g key={node.id}>
              <circle cx={pos.x} cy={pos.y} r="30" className={`${getColor(node.risk).replace("stroke", "fill").replace("500", "500/10")}`} />
      ...
      {data.nodes.map((node) => {
        ...
        return (
          ...
          >
            <div className="mb-8 pointer-events-none">{getIcon(node.type)}</div>
